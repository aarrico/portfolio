...
Elasticsearch. On a single node it holds 626 requests a second at a
44.8 ms p99 with nothing rejected. The whole stack runs locally in
Docker (Mongo, Elasticsearch, Redis, and the app) with
`make up && make seed`.

## Why three stores

MongoDB is the source of truth: it owns storage, filtered queries, and
the stats aggregations. Elasticsearch does full-text search and nothing
else. It only ever receives events Mongo has already accepted, so it
can't disagree with the source of truth. Redis is purely for
optimization: cache-aside realtime stats with a TTL, plus the rate-limit
counters. Hit `Kill Redis` above and the realtime endpoint recomputes
from Mongo instead of failing.
...
circuit breaker stops it consuming while Mongo is down. The queue would
buffer whatever it can hold, the client would get a `503 Retry-After` once
it filled, and nothing would reach the DLQ. As it stands, a short outage
leaves a pile of perfectly good messages to redrive by hand.
...
pretending everything is fine. Under sustained overload that is exactly
what happened: once the buffer filled, 57% of writes came back as
`503 Retry-After`, with zero errors and zero data loss among the events
that did get a `202`. Poison messages (payloads that can't be
...
with the process, and ingest can't scale separately from processing.

That coupling is measurable rather than theoretical. The workers are asyncio
tasks running on the API's own event loop, so they compete with in-flight
requests for it: drain ran about 15x faster with ingest idle than with it in
flight. More consumers help, but only so far. Throughput scaled close to
linearly from 50 to 251 events a second across one to four of them, and every
consumer I added bought drain rate at the cost of ingest p99. Batch size
isn't the lever either. It's capped at 10 to stay faithful to SQS's
`ReceiveMessage` limit, which trades throughput headroom for interface
fidelity.

Swap the queue for real SQS and the interface holds. What changes is the
ops story. The longer version of all this, including what each component
owns and how the system fails, is in the Starly repo's
[ARCHITECTURE.md](https://github.com/aarrico/starly/blob/take-home-submission/ARCHITECTURE.md).
